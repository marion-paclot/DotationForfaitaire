...
calculDF = function(annee, AnneeCreationCom, PopDGFnmoins1, PopDGFn, 
  ...
  MtEcretement = valeurPointn*PopDGFn*pmax(0,MtEcretement) # pmax si Exonéré si pf trop faible
  MtEcretement = pmin(MtEcretement, 0.01*RRFn, DFnmoins1 + PartDynPop) # Plafonnement
  
  MtEcretement = ifelse(AnneeCreationCom %in% c((annee-2):annee), 0, MtEcretement) # Garantie comm nouvelle
  MtEcretement = ifelse(DFnmoins1 == 0 & PartDynPop ==0, 0, MtEcretement) # Exoneration dotation forfaitaire annnée antérieure nulle
  MtEcretement = ifelse(PF4Taxesnmoins1 == 0, 0, MtEcretement) # Exoneration potentiel fiscal en cours nul
